...
if (qr && mcode) {
    ...
    const extraConfig = (typeof global.m_code === 'function') ? global.m_code(m.chat) : {};
...
try {
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathAssistantAccess)}@s.whatsapp.net`, {text : 'Advertencia: Sesión cerrada/no válida.' }, { quoted: m || null }) : ""
} catch (error) {}
...
m?.chat ? await conn.sendMessage(m.chat, { text: isSubBotConnected(m.sender) ? `@${m.sender.split('@')[0]}, ya estás conectado.` : `Has registrado un nuevo Sub-Bot: @${m.sender.split('@')[0]}`, mentions: [m.sender] }, { quoted: m }) : ''
}}
